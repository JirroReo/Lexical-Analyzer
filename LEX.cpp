...
using namespace std;
 ...
 //TODO Improve Regex, Add error detection
map<string,string> Make_Regex_Map(){
    map<string,string> my_map {       
        { "\\;|\\{|\\}|\\(|\\)|\\,|\\#", "Special Symbol"},
        { "int|char|float|bool","Data Type"},
        { "cin|cout|main|using|namespace|std","Keywords"},
...
int main()
{
    ...
    bool hasDataType = true; //True for debug, but should be false by default
    char c;
    string filename;
    ...
    for ( auto match = lang_matches.begin(); match != lang_matches.end(); ++match ){
        ...
        //     hasDataType = false;
        // } else if (match->second.second == "Data Type") {
        //     hasDataType = true;
        // }
        // Doesnt work, pls fix
        ...
        if(!(match->second.first==" ")&&!(match->second.first=="//")){
            ...
            if(count<10){
                ...
                if(hasDataType) {
                    ...
                    cout<<"\t Token   No :"<<double_digits<< "  |   "<<setw(10)<< match->second.first << " " <<" ------->  |"<< setw(25)<< match->second.second  <<setw(18)<< " ,  UNEXPECTED SYMBOL    "<< endl;
                    fout<<"\t Token   No :"<<double_digits<< "  |   "<<setw(10)<< match->second.first << " " <<" ------->  |"<< setw(25)<< match->second.second  <<setw(18)<< " ,  UNEXPECTED SYMBOL    "<< endl;
                    Sleep(1500);
                }
            }
            else{
                if (hasDataType) {
                    ...
                    cout<<"\t Token   No :"<<count<< "  |   "<< setw(10)<< match->second.first << " " <<" ------->  |"<< setw(25) << match->second.second  <<setw(18)<< " ,  UNEXPECTED SYMBOL    " << endl;
                    fout<<"\t Token   No :"<<count<< "  |   "<< setw(10)<< match->second.first << " " <<" ------->  |"<< setw(25) << match->second.second  <<setw(18)<< " ,  UNEXPECTED SYMBOL    " <<endl;
                    Sleep(1500);
                }
            }
            count++;
